...
export default function AgentMessagesPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-slate-100 flex flex-col" style={{ direction: lang === 'ar' ? 'rtl' : 'ltr' }}>
      {/* Header */}
      <header className="bg-[#131B2E] border-b border-slate-800 px-4 py-4 flex items-center gap-3 sticky top-0 z-40 shrink-0">
        <button onClick={() => router.back()} className="p-2 text-slate-400 hover:text-slate-100 rounded-xl"><ArrowLeft size={18} /></button>
        <div className="flex items-center gap-2 flex-1">
          <div className={`w-2.5 h-2.5 rounded-full ${adminOnline ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600'}`} />
          <div>
            <p className="font-black text-slate-100 text-sm">{t.subtitle}</p>
      ...
      {/* CHAT TAB */}
      {tab === 'chat' && (
        <div className="flex-1 flex flex-col min-h-0" style={{ height: 'calc(100vh - 70px)' }}>
          ...
          <form onSubmit={handleSend} className="flex gap-2 p-4 border-t border-slate-800 shrink-0">
